...
export class PhotosService {
  ...
  async startUpload(userId: string, albumId: string, dto: StartUploadDto) {
    ...
    const { photo, expiredKeys } = await this.prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        ...
        return { photo: existing, expiredKeys: [] as string[] };
      }
      ...
      if (stale.length) {
        await tx.photo.updateMany({
          ...
          data: { status: "FAILED", failureReason: "UPLOAD_EXPIRED" }
        });
      }
      const activeCount = await tx.photo.count({
        where: { albumId, albumDate, status: { in: [...ACTIVE_STATUSES] } }
      ...
      return {
        photo: created,
        expiredKeys: stale
          .map(({ tempObjectKey }) => tempObjectKey)
          .filter((key): key is string => Boolean(key))
      };
    });

    await Promise.allSettled(expiredKeys.map((key) => this.storage.delete(key)));
  ...
  async complete(userId: string, photoId: string) {
    ...
    if (photo.status === "READY") return this.cleanupTempObject(photo);
    ...
    try {
    const claimPhoto = (tx: PrismaService) => tx.photo.updateMany({
      where: {
        id: photo.id,
        OR: [
    ...
    const claim = photo.status === "FAILED"
      ? await this.prisma.$transaction(async (tx) => {
          ...
          assertDailyPhotoCapacity(activeCount);
          return claimPhoto(tx as PrismaService);
        })
    ...
    try {
      ...
      return await this.cleanupTempObject(ready);
    } catch (error) {
  ...
  private async cleanupTempObject(photo: { id: string; tempObjectKey: string | null }) {
    if (!photo.tempObjectKey) return photo;
    try {
      await this.storage.delete(photo.tempObjectKey);
      return await this.prisma.photo.update({
        where: { id: photo.id },
        data: { tempObjectKey: null, failureReason: null },
        include: { album: true, mediaAsset: true }
      });
    } catch {
      await this.prisma.photo.update({
        where: { id: photo.id },
        data: { failureReason: "TEMP_OBJECT_CLEANUP_PENDING" }
      }).catch(() => undefined);
      throw new ServiceUnavailableException({
  ...
  private async cleanupMediaAsset(mediaAssetId: string | null | undefined): Promise<void> {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (activeReferences > 0) return;

      const deleted = await Promise.allSettled([
        this.storage.delete(asset.originalKey),
        this.storage.delete(asset.displayKey),
        this.storage.delete(asset.thumbnailKey)
      ]);
