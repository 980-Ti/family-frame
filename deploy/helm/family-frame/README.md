...
Secret 이름과 각 key 이름은 `existingSecret.name` 및 `existingSecret.keys.*`로 변경할 수 있습니다. 현재 backend 코드가 직접 읽는 민감 설정은 `DATABASE_URL`과 S3 자격증명이며, `SESSION_SECRET`도 배포 Secret 계약에 포함해 backend에 주입합니다.

## 검증과 설치

```sh
helm lint deploy/helm/family-frame

helm template family-frame deploy/helm/family-frame \
  --namespace family-frame \
  > /tmp/family-frame-rendered.yaml

kubectl apply --dry-run=client \
  -f /tmp/family-frame-rendered.yaml

helm install family-frame deploy/helm/family-frame \
  --namespace family-frame \
  --dry-run \
  --debug
```
...
```sh
helm upgrade --install family-frame deploy/helm/family-frame \
  --namespace family-frame \
  --create-namespace \
...
frontend에는 `PORT`와 `API_ORIGIN`만 주입합니다. backend에는 `PORT`, ConfigMap 전체와 기존 Secret의 네 key를 주입합니다. `MAX_ACTIVE_UPLOADS_PER_USER`는 1~20 문자열이며 기본값은 `"5"`입니다.
...
기본 IngressClass는 `kong`이며 frontend `rakko.site`, backend `api.rakko.site`를 서로 다른 host rule로 렌더링합니다. `ingress.tls`가 비어 있으면 TLS block은 생성하지 않습니다. TLS Secret, 인증서, Cloudflare Tunnel, 외부 DNS, Ceph RGW와 PostgreSQL은 Chart 외부에서 관리합니다.
...
- `ingress.className`, 두 host와 환경별 TLS 설정
